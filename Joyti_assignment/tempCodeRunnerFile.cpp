...
int main() {
    ...
    do {
        ...
        cout<<"choose which book do you want to read from the library (index 0 to "<<books.size()<<")"<<endl;
        valid(index);
        ...
        for(size_t i=0;i<= books.size();i++) {
            cout<<"index "<<i<<"\nTittle: "<<books[index].gettittle()<<endl;
        cout<<"by "<<books[index].getauthor()<<endl;
        cout<<"cost "<<books[index].getprice()<<endl;
        }
        }
        ...
        cout<<"Choose form index from 0 to "<<books.size()<<endl;
        valid(index);
